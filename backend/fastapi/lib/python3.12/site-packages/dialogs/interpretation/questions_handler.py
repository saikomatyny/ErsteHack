...
class QuestionHandler(object):
                ...
                if statements:
                    ...
                    if self._process_on_location:
                        ...
                        roles = dict([(preposition, prepositions_list[preposition][0]) \
                                      for preposition in list(prepositions_list.keys()) \
        ...
        if sn:
            for sv in self._sentence.sv:
                for verb in sv.vrb_main:
                    ...
                    elif verb.lower() in list(ResourcePool().action_verb_with_passive_behaviour.keys()):
                        stmts.append(
    ...
    def get_concept_descriptor(self, ng):
        ...
        #Concept's Type
        elif not self._sentence.aim in list(QuestionAimDict().dic_aim.keys()):
...
class QuestionAimDict(object):
    def __init__(self):
        ...
        self.dic_on_direct_obj = dict(
            [(verb, role.id) for verb in list(verbs_dic.keys()) for role in verbs_dic[verb].roles[0:1]])
        self.dic_on_direct_obj[None] = 'involves'
        self.dic_on_direct_obj['be'] = 'owl:sameAs'

        self.dic_on_indirect_obj = dict(
            [(verb, role.id) for verb in list(verbs_dic.keys()) for role in verbs_dic[verb].roles[1:2]])
